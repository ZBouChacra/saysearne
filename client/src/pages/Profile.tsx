    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <div className="container py-8 flex-1">
        ...
        <Tabs defaultValue="personal">
          ...
          {isProfessional && <TabsContent value="professions" className="mt-4"><ProfessionsManager days={DAYS} /></TabsContent>}
          {isProfessional && <TabsContent value="availability" className="mt-4"><AvailabilityManager days={DAYS} /></TabsContent>}
        </Tabs>
      </div>
      <Footer />
    </div>
...
function PersonalInfoForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="border border-border bg-card p-6 space-y-4 max-w-2xl">
      {/* Profile Type Toggle */}
      <div className="border border-primary/20 bg-primary/5 p-4 mb-2">
        <Label className="font-semibold">{t("profile.profileType")}</Label>
        ...
        <p className="text-xs text-muted-foreground mt-1">{t("profile.profileTypeHint")}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <Label>{t("profile.dateOfBirth")} (dd/MM/yyyy)</Label>
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogTrigger asChild><Button className="gap-2"><Plus className="h-4 w-4" /> {t("profile.addService")}</Button></DialogTrigger>
          <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><Label>{t("profile.costPerHour")}</Label><Input type="number" value={newProf.costPerHour} onChange={(e) => setNewProf(f => ({ ...f, costPerHour: e.target.value }))} className="mt-1.5" /></div>
                <div><Label>{t("profile.yearsOfExperience")}</Label><Input type="number" value={newProf.yearsOfExperience || ""} onChange={(e) => setNewProf(f => ({ ...f, yearsOfExperience: Number(e.target.value) }))} className="mt-1.5" /></div>
              </div>
              <div><Label>{t("profile.website")}</Label><Input value={newProf.website} onChange={(e) => setNewProf(f => ({ ...f, website: e.target.value }))} className="mt-1.5" placeholder="https://..." /></div>
              ...
              <div className="flex items-center gap-4">
                ...
                {newProf.hasTeam && <div className="flex items-center gap-2"><Label>{t("profile.teamSize")}:</Label><Input type="number" className="w-20" value={newProf.teamSize || ""} onChange={(e) => setNewProf(f => ({ ...f, teamSize: Number(e.target.value) }))} /></div>}
              </div>

              <div className="border border-border p-4 space-y-3 bg-muted/30">
                <div className="flex items-center gap-2"><Switch checked={newProf.hasOffice} onCheckedChange={(v) => setNewProf(f => ({ ...f, hasOffice: v }))} /><Label className="font-semibold text-sm flex items-center gap-2"><Building2 className="h-4 w-4" /> {t("profile.hasOffice")}</Label></div>
                {newProf.hasOffice && (
                  <div className="space-y-3">
                    <div><Label>{t("profile.officeAddress")}</Label><Input value={newProf.officeAddress} onChange={(e) => setNewProf(f => ({ ...f, officeAddress: e.target.value }))} className="mt-1.5" /></div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
              ...
              <div><Label>{t("profile.geographicAreas")}</Label><Input value={newProf.geographicAreas} onChange={(e) => setNewProf(f => ({ ...f, geographicAreas: e.target.value }))} className="mt-1.5" placeholder="New York, Los Angeles" /></div>
              <Button onClick={handleAdd} className="w-full" disabled={addMutation.isPending}>{addMutation.isPending ? t("profile.adding") : t("profile.addService")}</Button>
            </div>
          </DialogContent>
        </Dialog>
      </div>
            ...
            return (
              <div key={prof.id} className="border border-border bg-card p-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{lang === "ar" && cat?.nameAr ? cat.nameAr : cat?.name} &middot; ${prof.costPerHour}/hr &middot; {prof.yearsOfExperience}y exp</p>
                  <p className="text-xs text-muted-foreground flex items-center gap-1 mt-1">
                    <MapPin className="h-3 w-3" /> {[prof.city, prof.country].filter(Boolean).join(", ") || t("profile.noLocation")}
                    {prof.hasOffice && <span className="ms-2 flex items-center gap-1"><Building2 className="h-3 w-3" /> Office</span>}
                    {prof.hasTeam && prof.teamSize && <span className="ms-2">&middot; Team: {prof.teamSize}</span>}
                  </p>
                </div>
                <Button variant="ghost" size="icon" className="text-destructive shrink-0" onClick={() => deleteMutation.mutate({ id: prof.id })}><Trash2 className="h-4 w-4" /></Button>
              </div>
            );
          })}
        </div>
      ) : (
        <div className="text-center py-12 border border-dashed border-border"><Briefcase className="h-12 w-12 mx-auto text-muted-foreground/50 mb-4" /><p className="text-muted-foreground">{t("profile.noServicesYet")}</p></div>
      )}
    </div>
  );
}
        ...
        <div className="space-y-3 mb-4">
          {slots.map((slot, i) => (
            <div key={i} className="flex items-center gap-3 border border-border bg-card p-3">
              ...
              <Button variant="ghost" size="icon" onClick={() => removeSlot(i)} className="text-destructive shrink-0"><Trash2 className="h-4 w-4" /></Button>
            </div>
          ))}
        </div>
      ) : (
        <div className="text-center py-8 border border-dashed border-border mb-4"><p className="text-muted-foreground">{t("profile.noSlots")}</p></div>
      )}

      <Button onClick={() => saveMutation.mutate({ professionId: selectedProfId, slots })} className="gap-2" disabled={saveMutation.isPending}>
